class TestLineItem < ApplicationRecord
  ...
  def TestLineItem.generate(eng_word, vocabulary_test)
    line_item = vocabulary_test.test_line_items.create eng_word: eng_word
    ...
    line_item.test_options.create option: right_answers.sample.word
    other_words.each do |another_rus_word|
      line_item.test_options.create option: another_rus_word.word
    end
  end
end
